...
public class FindShortestPathToANumber {
	/*
	 * given a special series which starts from 1 and has the number at position
			...
			for (int i = 0; i < arr.size(); i++) {
				for (int j = 0; j < arr.size(); j++) {
					...
					if (!arr.contains(num) && num <= N) {
						ArrayList<Integer> tmp = (ArrayList<Integer>) arr
								.clone();
						tmp.add(num);
	...
	public MyNode FindWithTreeStructure(int N, ArrayList<MyNode> candidate) {
		// candidate is the leaf layer nodes of the tree
		...
		for (MyNode n : candidate) {
			...
			while (n1 != null) {
				...
				while (n2 != null) {
					...
					if (value == N) {
						// search layer by layer. The first hit is the answer
						return new MyNode(value, n);
					} else if (value < N && value > n.Value
							&& !tmp.contains(value)) {
						// don't have to look at numbers that are larger than N
						// since all the number is the series are positive
