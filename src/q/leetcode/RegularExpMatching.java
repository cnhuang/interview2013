...
public class RegularExpMatching extends Question {
	...
	public boolean isMatch_efficient(String s, String p) {
		...
		for (int i = 1; i < n; i++) {
			for (int j = 1; j < m; j++) {
				if (p.charAt(i - 1) == '*')
					result[i][j] = isMatch(s.charAt(j - 1), p.charAt(i - 2)) ?

					// either take previous result (so aaA will mathc a* but not
					// abA , since A take b's result)
					...
					result[i][j] = isMatch(s.charAt(j - 1), p.charAt(i - 1)) ?
